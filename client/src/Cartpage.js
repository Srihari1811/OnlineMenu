...
function CartPage() {
  ...
  const fetchCartData = async () => {
    try {
      const response = await axios.get(`https://online-menu-api.vercel.app/cart/${userId}`);
  ...
  const updateQuantity = async (id, change) => {
    try {
      ...
      await axios.post(`https://online-menu-api.vercel.app/cart/${userId}`, { cart: updatedCart });
    } catch (error) {
