...
function CategoryProducts() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`https://online-menu-api.vercel.app/addproducts/${categoryId}`);
  ...
  return (
    <div className="container mt-4">
      <div className="navigation-container d-flex justify-content-between align-items-center mb-4">
        ...
        <h2 className="products-title">Items</h2>
      ...
      {showAlert && (
        <div className="fixed-bottom d-flex justify-content-center">
          <div 
            className="alert alert-success mb-3 d-flex align-items-center justify-content-between" 
            role="alert"
          >
            <span>{`${addedItemsCount} item${addedItemsCount > 1 ? 's' : ''} added to cart.`}</span>
            <Link to="/cart" className="text-white font-weight-bold ml-2">
              View Cart
            </Link>
          </div>
        </div>
