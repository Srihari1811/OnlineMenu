...
function AddCategory({ onCategoryAdded, onNextClick }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editMode) {
      ...
      try {
        ...
        const response = await axios.put(`https://online-menu-api.vercel.app/updateCategory/${editCategoryId}`, formData);
      ...
      try {
        ...
        const response = await axios.post('https://online-menu-api.vercel.app/addCategories', formData);
  ...
  const handleRemove = async (categoryId) => {
    try {
      const response = await axios.delete(`https://online-menu-api.vercel.app/deleteCategory/${categoryId}`);
      if (response.status === 200) {
